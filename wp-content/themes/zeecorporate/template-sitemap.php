...
<?php get_header(); ?>
	...
	<div id="content">
		...
		<?php wp_reset_query(); ?> 
				...
				<h2><?php _e('Рубрики', 'themezee_lang'); ?></h2><br/>
				<ul>
					<?php wp_list_categories('title_li=&show_count=1'); ?>
				</ul>
					
				<h2><?php _e('Архивы', 'themezee_lang'); ?></h2><br/>
				<ul>
					<?php wp_get_archives('show_post_count=true'); ?>
				</ul>
					
				<h2><?php _e('Подшивка по рубрикам', 'themezee_lang'); ?></h2><br/>
					<?php $categories = get_categories(); ?>
					<?php foreach($categories as $cat) : ?>
						<strong><?php _e('Рубрика', 'themezee_lang'); ?>: <a href="<?php echo get_category_link( $cat->term_id ); ?>"><?php echo $cat->name; ?></a></strong>
							<ul>
